 ...
 * Persistence with Queues tutorial - Replay Queue Consumer
 * Demonstrates receiving replayed persistent messages from a queue
 */

/*jslint es6 browser devel:true*/
/*global solace*/

var ReplayQueueConsumer = function (queueName) {
    'use strict';
    var consumer = {};
    ...
    consumer.startConsume = function () {
        if (consumer.session !== null) {
            if (consumer.consuming) {
                ...
                try {
                    // Create a ReplayStartLocation
                    var date = document.getElementById('replay-start-location').value;
                    var replayStartLocation;
                    if(!date) {
                        ...
                        try {
                            ...
                            if(inputDateTime > currentDateTime) {
                                console.log("Replay start time is from the future!  Retreiving all messages...")
                                replayStartLocation = solace.SolclientFactory.createReplayStartLocationBeginning();
                            } else {
                                replayStartLocation = solace.SolclientFactory.createReplayStartLocationDate(inputDateTime);
                            }
                        } catch {
                            console.log('Replay start time input not formatted correctly.')
                            return;
                        }
                    }
                    ...
                    console.log(consumer.messageConsumer);
